...
from nets.utils import make_anchors

import torch
from torchvision.ops import nms

class YOLODetector(torch.nn.Module):
    def __init__(self, model_size="x", num_classes=80, conf_thresh=0.2, iou_thresh = 0.8,device="cuda"):
        super().__init__()
        # 选择 YOLOv11 backbone
        if model_size == "n":
            self.model = yolo_v11_n(num_classes)
        elif model_size == "t":
            self.model = yolo_v11_t(num_classes)
        elif model_size == "s":
            self.model = yolo_v11_s(num_classes)
        elif model_size == "m":
            self.model = yolo_v11_m(num_classes)
        elif model_size == "l":
            self.model = yolo_v11_l(num_classes)
        elif model_size == "x":
            self.model = yolo_v11_x(num_classes)
        else:
            raise ValueError(f"Unsupported YOLO v11 model size: {model_size}")
    ...
    def forward(self, images):
        ...
        输入:
            images: [B,3,H,W]  (像素空间)
        输出:
            detections: (boxes_list, scores_list, classes_list)
                boxes_list: list length B, each: [N,4] (x1,y1,x2,y2) 归一化到 [0,1]
                scores_list: list length B, each: [N]
                classes_list: list length B, each: [N] (int64)
        """
        B, _, H, W = images.shape
        images = images.to(self.device)

        p3, p4, p5 = self.model.net(images)   # DarkNet 输出
        fpn_feats = self.model.fpn((p3, p4, p5))  # 融合后的特征 (p3, p4, p5)

        # 2. 预测
        preds = self.model.head(list(fpn_feats))   # [B, num_outputs, num_anchors]
        boxes_list = []
        scores_list = []
        classes_list = []

        for b in range(B):
            pred = preds[b].permute(1, 0)  # -> [num_anchors, no]
            box = pred[:, :4]   # (cx,cy,w,h)，单位是像素
            cls = pred[:, 4:]   # [num_anchors, num_classes]
            ...
            # 归一化到 [0,1]
            norm = torch.tensor([W, H, W, H], device=box.device, dtype=box.dtype)
            boxes_xyxy = torch.stack([x1, y1, x2, y2], dim=-1) / norm

            # 每个 anchor 的最高类别
            conf, cls_id = cls.max(dim=1)  # conf: [num_anchors], cls_id: [num_anchors]

            # 阈值过滤
            mask = conf > self.conf_thresh
            boxes_keep = boxes_xyxy[mask]
            scores_keep = conf[mask]
            classes_keep = cls_id[mask]

            # boxes_list.append(boxes_keep)
            # scores_list.append(scores_keep)
            # classes_list.append(classes_keep.long())
            final_boxes = []
            final_scores = []
            final_classes = []

            for c in range(self.num_classes):
                inds = (classes_keep == c).nonzero(as_tuple=False).squeeze(1)
                if inds.numel() == 0:
                    continue
                cls_boxes = boxes_keep[inds]
                cls_scores = scores_keep[inds]

                # torchvision.ops.nms 需要 [x1,y1,x2,y2]，且 IoU 阈值在[0,1]
                keep = nms(cls_boxes, cls_scores, self.iou_thresh)
                if keep.numel() == 0:
                    continue
                final_boxes.append(cls_boxes[keep])
                final_scores.append(cls_scores[keep])
                final_classes.append(torch.full((keep.numel(),), c, dtype=torch.long, device=images.device))

            if len(final_boxes) == 0:
                boxes_list.append(torch.zeros((0, 4), device=images.device))
                scores_list.append(torch.zeros((0,), device=images.device))
                classes_list.append(torch.zeros((0,), dtype=torch.long, device=images.device))
            else:
                boxes_cat = torch.cat(final_boxes, dim=0)
                scores_cat = torch.cat(final_scores, dim=0)
                classes_cat = torch.cat(final_classes, dim=0)
                order = scores_cat.argsort(descending=True)
                boxes_list.append(boxes_cat[order])
                scores_list.append(scores_cat[order])
                classes_list.append(classes_cat[order])

        detections = (boxes_list, scores_list, classes_list)
        feature_maps = fpn_feats  # [p3, p4, p5]
