...
class YOLODetector(nn.Module):
    def __init__(self, model_size="x", num_classes=80, conf_thresh=0.25, iou_thresh=0.45, device="cuda"):
        super().__init__()
        # 选择 YOLOv8 backbone
        if model_size == "n":
            self.model = yolo_v8_n(num_classes)
        elif model_size == "s":
            self.model = yolo_v8_s(num_classes)
        elif model_size == "m":
            self.model = yolo_v8_m(num_classes)
        elif model_size == "l":
            self.model = yolo_v8_l(num_classes)
        elif model_size == "x":
            self.model = yolo_v8_x(num_classes)
        else:
            raise ValueError(f"Unsupported YOLO model size: {model_size}")
    ...
    def forward(self, images):
        ...
        输出:
            ...
            feature_maps: (p3, p4, p5) 来自 FPN 的多尺度特征
        """
        ...
        # preds 形状: [B, 4+num_classes, num_anchors]
        boxes, scores, classes = [], [], []

        for b in range(B):
            pred = preds[b]  # [4+num_classes, num_anchors]
            pred = pred.transpose(0, 1)  # [num_anchors, 4+num_classes]
            box = pred[:, :4]  # (cx, cy, w, h)
            cls = pred[:, 4:]  # class logits
            ...
            x2 = box[:, 0] + box[:, 2] / 2
            y2 = box[:, 1] + box[:, 3] / 2
            box = torch.stack([x1, y1, x2, y2], dim=-1) / torch.tensor([W, H, W, H], device=box.device)

            # 置信度
            conf, cls_id = cls.max(dim=-1)
            mask = conf > self.conf_thresh

            boxes.append(box[mask])
            scores.append(conf[mask])
            classes.append(cls_id[mask])
...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=()):
        ...
        self.cls = torch.nn.ModuleList(torch.nn.Sequential(Conv(1, c1, 3),
                                                           Conv(1, c1, 3),
                                                           torch.nn.Conv2d(1, self.nc, 1)) for x in filters)
        self.box = torch.nn.ModuleList(torch.nn.Sequential(Conv(1, c2, 3),
                                                           Conv(1, c2, 3),
                                                           torch.nn.Conv2d(1, 4 * self.ch, 1)) for x in filters)
